    ...
    def post(self, request, post_id):
        ...
        try:
            ...
            if request.user in post.liked_users.all():
                ...
                post.likes_count -= 1
                status = 'unliked'
                print(f"User unliked the post")
            else:
                post.liked_users.add(request.user)
                post.likes_count += 1
                status = 'liked'
                print(f"User liked the post")
    ...
    def get(self, request, user_id=None, group_id=None):
        ...
        if group_id:
            ...
            try:
                ...
                print(f"Found {posts.count()} posts in group {group_id} between {start_date} and {end_date}")
            except Group.DoesNotExist:
            ...
            try:
                ...
                if selected_group_id:
                    ...
                    try:
                        ...
                        print(f"Found {posts.count()} posts by user {user_id} in group {selected_group_id} between {start_date} and {end_date}")
                    except Group.DoesNotExist:
                    ...
                    print(f"Found {posts.count()} posts from user's groups {[g.name for g in user_groups]} between {start_date} and {end_date}")
        ...
        for post in posts:
            print(f"Processing post {post.id}: {post.product_name} - {post.customer_category} - {post.created_at}")
