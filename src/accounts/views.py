...
class DeleteAccountView(View):
    def get(self, request):
        ...
        try:
            if user.is_authenticated:
                logger.info(f"User is authenticated, custom_user_id: {getattr(user, 'custom_user_id', 'Not found')}")
                fresh_user = CustomUser.objects.get(custom_user_id=user.custom_user_id)
                logger.info(f"Fresh user retrieved: {type(fresh_user)}, ID: {fresh_user.custom_user_id}")
            else:
        ...
        context = {
            ...
            'user_type': type(fresh_user).__name__ if fresh_user else type(user).__name__,
            'verification_result': None
        }
        
        logger.info(f"Final context user: {type(context['user'])}")
        if context['user']:
            logger.info(f"Final user ID: {getattr(context['user'], 'custom_user_id', 'Not found')}")
    ...
    def post(self, request):
        ...
        try:
            if user.is_authenticated:
                logger.info(f"POST - User is authenticated, custom_user_id: {getattr(user, 'custom_user_id', 'Not found')}")
                fresh_user = CustomUser.objects.get(custom_user_id=user.custom_user_id)
                logger.info(f"POST - Fresh user retrieved: {type(fresh_user)}, ID: {fresh_user.custom_user_id}")
            else:
        ...
        logger.info(f"Password1 field value: {getattr(fresh_user, 'password1', 'Not found')}")
        logger.info(f"Standard password field exists: {hasattr(fresh_user, 'password')}")
        logger.info(f"Standard password field value: {getattr(fresh_user, 'password', 'Not found')}")
        ...
        if hasattr(fresh_user, 'password1') and fresh_user.password1:
            try:
                if check_password(password, fresh_user.password1):
                    ...
                    logger.info("Password verification successful using password1 field")
                else:
        ...
        if not password_verified:
            try:
                if fresh_user.check_password(password):
                    ...
                    logger.info("Password verification successful using Django's check_password")
                else:
                    logger.info("Password verification failed using Django's check_password")
            except Exception as e:
                logger.error(f"Error checking password with Django's method: {e}")
        
        if not password_verified and hasattr(fresh_user, 'password1'):
            try:
                latest_user = CustomUser.objects.get(custom_user_id=fresh_user.custom_user_id)
                if check_password(password, latest_user.password1):
                    password_verified = True
                    verification_method = "latest user data"
                    logger.info("Password verification successful using latest user data")
                else:
        ...
        if password_verified:
            logger.info(f"Password verification successful for user: {fresh_user.custom_user_id}")
            try:
                ...
                try:
                    check_user = CustomUser.objects.get(custom_user_id=fresh_user.custom_user_id)
                    logger.warning(f"User still exists after deletion: {check_user.custom_user_id}")
                except CustomUser.DoesNotExist:
                    logger.info(f"User {fresh_user.custom_user_id} confirmed deleted from database")
            ...
            logger.warning(f"All password verification methods failed for user: {fresh_user.custom_user_id}")
